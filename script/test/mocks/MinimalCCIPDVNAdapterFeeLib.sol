...
/// Minimal, non-upgradeable replacement for LZ's `CCIPDVNAdapterFeeLib`. The
/// upstream version inherits `OwnableUpgradeable` + `Proxied` (hardhat-deploy
/// proxy pattern), neither of which we need for a one-shot deploy. Returns the
/// raw CCIP router fee with the adapter's `defaultMultiplierBps` premium; no
/// floor margin, no per-dst override.
contract MinimalCCIPDVNAdapterFeeLib is ICCIPDVNAdapterFeeLib {
